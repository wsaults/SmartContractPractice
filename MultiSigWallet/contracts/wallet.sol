...
contract MyWallet is mortal {
    ...
    event proposalReceived(address indexed _from, address indexed _to, string reason);
    ...
    function spendMondayOn(address _to, uint256 _value, string _reason) returns (uint256) {
        if (owner == msg.sender) {
